...
def train_and_evaluate_shapes(
        ...
        hyperparameters=None, fast=False, device=None, non_blocking=False, seed=None, verbose=1):
    """
    ...
    for i in range(len(models)):
        ...
        if model.name == "ShapesCNN":
            history, models_dict = train_simple(
                ...
                n_early_stop=None, device=device, non_blocking=non_blocking, verbose=verbose)
        else:
            history, models_dict = train_cbm(
                ...
                device=device, non_blocking=non_blocking, verbose=verbose)
...
def run_models_on_subsets_and_plot(
    ...
    for n_subset in subsets:
        seed = base_seed
        if verbose != 0:
            print(f"Beginning subset {n_subset}:")
        ...
        for i in range(n_bootstrap):
            ...
            histories = train_and_evaluate_shapes(
                ...
                test_loader=test_loader, hyperparameters=None, fast=fast, device=device, non_blocking=non_blocking,
